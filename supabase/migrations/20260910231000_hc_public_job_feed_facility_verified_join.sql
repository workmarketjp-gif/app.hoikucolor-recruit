-- Expose only the facility key needed to join a public job to its separately
-- publication-controlled Hoiku Office verified workplace profile.
-- No raw workplace snapshot fields are exposed here.
...
-- Public feed is read-only to browser roles.
revoke all on public.hc_public_job_feed from anon, authenticated;
