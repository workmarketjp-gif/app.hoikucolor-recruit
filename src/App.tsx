...
function JobCard({ job, saved, onToggleSaved }: { job: Job; saved: boolean; onToggleSaved: (id: string) => void }) {
  ...
  return <article className="job-card" data-job-id={job.id}>
    ...
    <div className="job-tags">{isClosed && <span className="status-badge status-rejected">募集終了</span>}<span>園掲載</span>{job.employment_type && <span>{job.employment_type}</span>}{job.facility_type && <span>{job.facility_type}</span>}{job.verified_workplace?.verified_metric_count ? <span className="verified-tag">✓ Hoiku Office 実績</span> : <span>HO実績データなし</span>}{job.verified_finance?.verified_metric_count ? <span className="finance-verified-tag">✓ Hoiku Finance 実績</span> : <span>HF実績データなし</span>}</div>
