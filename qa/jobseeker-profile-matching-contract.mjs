...
expect(rpcBoundary.includes('public.hc_jobseeker_get_matching_preferences()'), 'matching preference read RPC source must be versioned in Git');
expect(rpcBoundary.includes('public.hc_jobseeker_upsert_matching_preferences('), 'matching preference write RPC source must be versioned in Git');
expect(rpcBoundary.includes("nullif((select auth.jwt()) ->> 'sub', '')"), 'matching RPC boundary must derive Candidate identity from JWT');
expect(rpcBoundary.includes('to authenticated;'), 'matching RPC boundary must remain authenticated-only');
expect(repository.includes("rpc('hc_jobseeker_get_matching_preferences')"), 'matching preferences must read through JWT-owned RPC');
expect(repository.includes("rpc('hc_jobseeker_upsert_matching_preferences'"), 'matching preferences must write through JWT-owned RPC');
expect(!repository.includes(".from('hc_jobseeker_profiles')"), 'matching preferences must not directly access profile PII table');
