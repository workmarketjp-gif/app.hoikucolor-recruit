...
const checks = [
  ...
  [enhancer.includes("window.setInterval(refreshWhenVisible, 60_000)"), 'attention state must refresh periodically while visible'],
  [enhancer.includes("window.addEventListener('focus', refreshWhenVisible)"), 'attention state must refresh on browser focus'],
  [enhancer.includes("window.addEventListener('pageshow', refreshWhenVisible)"), 'attention state must refresh after mobile/back-forward cache restore'],
  [enhancer.includes("document.addEventListener('visibilitychange', refreshWhenVisible)"), 'attention state must refresh when a hidden tab becomes visible'],
  [enhancer.includes("window.removeEventListener('focus', refreshWhenVisible)") && enhancer.includes("window.removeEventListener('pageshow', refreshWhenVisible)"), 'attention resume listeners must clean up'],
