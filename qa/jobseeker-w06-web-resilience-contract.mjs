...
const checks = [
  ...
  ['online data failure exposes in-page retry', connectivity.includes("document.querySelector('.error-banner')") && connectivity.includes('MutationObserver') && connectivity.includes("online ? '再試行' : '再読み込み'")],
  ['online recovery keeps a clear next action', connectivity.includes('データの取得に失敗しました。再試行してください。')],
  ['recovery is accessible', connectivity.includes('role="status"') && connectivity.includes('aria-live="polite"')],
  ['recovery stays usable on narrow mobile', connectivityCss.includes('@media(max-width:460px)') && connectivityCss.includes('min-height:44px')],
