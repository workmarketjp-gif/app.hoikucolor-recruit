...
const checks = [
  ...
  ['client deletion uses owner-bound prepare RPC', /rpc\('hc_prepare_jobseeker_document_delete'[\s\S]*p_document_id:\s*document\.id/i.test(repository)],
  ['client deletion finalizes only after Storage attempt', /remove\(\[filePath\]\)[\s\S]*rpc\('hc_finalize_jobseeker_document_delete'/i.test(repository)],
